...
#define H5_APP_DISPLAY_PERIOD_MS       (100U)
#define H5_APP_UART_BAUD               (115200UL)
#define H5_APP_UART_PRIORITY           (2U)
#define H5_APP_BALL_READY_ERROR_CM     H5_BALL_READY_ERROR_CM
#define H5_APP_BALL_READY_SPEED_CM_S   H5_BALL_READY_SPEED_CM_S
#define H5_APP_BALL_READY_SETTLE_MS    H5_BALL_READY_SETTLE_MS
#define H5_APP_BALL_SCORE_ERROR_CM     H5_BALL_SCORE_ERROR_CM
#define H5_APP_BALL_SCORE_IMMEDIATE_CM H5_BALL_SCORE_IMMEDIATE_CM
#define H5_APP_BALL_SCORE_CONFIRM_MS   H5_BALL_SCORE_CONFIRM_MS
#define H5_APP_BALL_ABORT_ERROR_CM     H5_BALL_ABORT_ERROR_CM
#define H5_APP_BALL_ABORT_HOLD_MS      H5_BALL_ABORT_HOLD_MS
#define H5_APP_LAUNCH_BIAS_US          H5_LAUNCH_BIAS_US
#define H5_APP_LAUNCH_BIAS_MS          H5_LAUNCH_BIAS_MS
#define H5_APP_LINE_CORRECTION_RATIO   H5_LINE_CORRECTION_RATIO
#define H5_APP_LINE_CORRECTION_MAX_MM_S H5_LINE_CORRECTION_MAX_MM_S
#define H5_APP_OUTER_CORRECTION_RATIO  H5_LINE_OUTER_CORRECTION_RATIO
...
typedef struct {
    ...
    h5_mission_output_t mission_output;
...
static const icm42688_service_config_t g_h5_imu_config = {
    {
        {
            IMU_ATTITUDE_AXIS_X,
            IMU_ATTITUDE_AXIS_Y,
...
static h5_app_context_t g_app;

static float h5_app_abs(float value)
{
    return value < 0.0f ? -value : value;
}

static bool h5_key_pressed(
    GPIO_Regs *port, uint32_t pin)
{
    return gpio_get(port, pin) == ML_KEY_ACTIVE_LEVEL;
}

static ml_status_t h5_keys_init(void)
{
    ml_status_t status;
...
static void h5_show_line(uint8_t line, const char *text)
{
...
static int32_t h5_tenths(float value)
{
...
static const char *h5_setup_wait_text(void)
{
    ...
    if (h5_app_abs(g_app.ball.error_cm) >
        H5_APP_BALL_READY_ERROR_CM) {
        return "WAIT POS <=0.8";
    }
    if (h5_app_abs(g_app.ball.velocity_cm_per_s) >
        H5_APP_BALL_READY_SPEED_CM_S) {
        return "WAIT SPD <=1.0";
    }
    return "SETTLE 500MS";
}

static void h5_show_boot(void)
{
    h5_show_line(1U, "H5 BOOT");
    h5_show_line(2U, "INIT MODULES");
    h5_show_line(3U, "MOTORS SAFE");
    h5_show_line(4U, "WAIT...");
}

static void h5_show_init_failure(ml_status_t status, const char *detail)
{
    char line[OLED_TEXT_COLUMN_COUNT + 1U];

    h5_show_line(1U, "H5 INIT FAIL");
    (void) snprintf(line, sizeof(line), "STATUS %02u",
        (unsigned int) status);
    h5_show_line(2U, line);
    h5_show_line(3U, "MOTORS SAFE");
    h5_show_line(4U, detail == 0 ? "CHECK MODULES" : detail);
}

static void h5_show(void)
{
    char line[OLED_TEXT_COLUMN_COUNT + 1U];
    int32_t target_tenths = h5_tenths(g_app.target_cm);
    int32_t ball_tenths = h5_tenths(g_app.ball.position_cm);
    uint32_t time_ms = g_app.mission_output.score_elapsed_ms;
    bool score_pass = g_app.mission_output.score_point_passed &&
        (g_app.mission_output.result == H5_MISSION_RESULT_PASS) &&
        !g_app.ball_violation;

    if (g_app.state == H5_APP_CALIBRATING) {
        h5_show_line(1U, "H5 CALIBRATE");
        ...
        h5_show_line(3U, line);
        h5_show_line(4U, "WAIT READY SAFE");
    } else if ((g_app.state == H5_APP_SETUP) ||
               (g_app.state == H5_APP_READY)) {
        (void) snprintf(line, sizeof(line), "H%u %s",
            (unsigned int) g_app.mode,
            g_app.state == H5_APP_READY ? "READY" : "BALL SETUP");
        h5_show_line(1U, line);
        (void) snprintf(line, sizeof(line), "T%+03ld.%01ld P%+03ld.%01ld",
            (long) (target_tenths / 10),
            (long) h5_app_abs((float) (target_tenths % 10)),
            (long) (ball_tenths / 10),
            (long) h5_app_abs((float) (ball_tenths % 10)));
        h5_show_line(2U, line);
        h5_show_line(3U, "TARGET FIXED 0");
        h5_show_line(4U, g_app.state == H5_APP_READY ?
            "C=GO U/D=MODE" : h5_setup_wait_text());
    } else if (g_app.state == H5_APP_FINISHED) {
        ...
        (void) snprintf(line, sizeof(line), "BALL MAX %02ld.%01ld",
            (long) (h5_tenths(g_app.maximum_score_error_cm) / 10),
            (long) h5_app_abs((float)
                (h5_tenths(g_app.maximum_score_error_cm) % 10)));
        h5_show_line(2U, line);
        h5_show_line(3U, "C=SAFE RECENTER");
        (void) snprintf(line, sizeof(line), "CSV N%03u WAIT",
            (unsigned int) h5_telemetry_count());
        h5_show_line(4U, line);
    } else if (g_app.state == H5_APP_RECENTERING) {
        h5_show_line(1U, g_app.fault == H5_APP_FAULT_NONE ?
            "H5 RECENTER" : "H5 FAULT SAFE");
        ...
        h5_show_line(2U, line);
        h5_show_line(3U, "MOTORS LOCKED");
        h5_show_line(4U, "WAIT 1500 US");
    } else if ((g_app.state == H5_APP_EXPORT_READY) ||
               (g_app.state == H5_APP_FAULT)) {
        (void) snprintf(line, sizeof(line), "H5 %s F%u",
            g_app.fault == H5_APP_FAULT_NONE ? "SAFE" : "FAULT",
        ...
        (void) snprintf(line, sizeof(line), "T%02lu.%01lu M%02ld.%01ld",
            ...
            (long) h5_app_abs((float)
                (h5_tenths(g_app.maximum_score_error_cm) % 10)));
        h5_show_line(2U, line);
        h5_show_line(3U, "MOTORS+SERVO SAFE");
...
static void h5_update_keys(uint32_t now_ms)
{
    chassis_key_update(&g_app.center_key,
        h5_key_pressed(ML_KEY_CENTER_PORT, ML_KEY_CENTER_PIN), now_ms);
    chassis_key_update(&g_app.up_key,
        h5_key_pressed(ML_KEY_UP_PORT, ML_KEY_UP_PIN), now_ms);
    chassis_key_update(&g_app.down_key,
        h5_key_pressed(ML_KEY_DOWN_PORT, ML_KEY_DOWN_PIN), now_ms);
    chassis_key_update(&g_app.left_key,
        h5_key_pressed(ML_KEY_LEFT_PORT, ML_KEY_LEFT_PIN), now_ms);
    chassis_key_update(&g_app.right_key,
        h5_key_pressed(ML_KEY_RIGHT_PORT, ML_KEY_RIGHT_PIN), now_ms);
}

static void h5_require_key_release(void)
{
...
static ml_status_t h5_configure_line_control(h5_mode_t mode)
{
    ...
    (void) mode;
    line_config.correction_ratio = H5_APP_LINE_CORRECTION_RATIO;
    line_config.maximum_correction_mm_s = H5_APP_LINE_CORRECTION_MAX_MM_S;
    line_config.outer_single_correction_ratio =
        H5_APP_OUTER_CORRECTION_RATIO;
    line_config.outer_single_maximum_correction_mm_s =
        H5_APP_OUTER_CORRECTION_MAX_MM_S;
...
static void h5_reset_settle(void)
{
    g_app.ball_settle_active = false;
    g_app.ball_settled = false;
    g_app.ball_settle_start_ms = 0U;
    if (g_app.state == H5_APP_READY) {
        g_app.state = H5_APP_SETUP;
    }
}

static void h5_clear_launch_bias(void)
{
...
static void h5_start_launch_bias(uint32_t now_ms)
{
    g_app.launch_bias_us = H5_APP_LAUNCH_BIAS_US;
    g_app.launch_bias_duration_ms = H5_APP_LAUNCH_BIAS_MS;
    g_app.launch_bias_active = true;
    g_app.launch_bias_start_ms = now_ms;
    (void) ball_balance_set_control_bias_us(g_app.launch_bias_us);
}

static void h5_update_launch_bias(uint32_t now_ms)
{
    uint32_t elapsed_ms;
    float scale;
    ...
    if (g_app.state != H5_APP_RUNNING) {
        h5_clear_launch_bias();
        return;
    }
...
static void h5_update_ball_settle(uint32_t now_ms)
{
    bool stable = g_app.ball.vision_ready && g_app.ball.enabled &&
        (g_app.ball.state == BALL_BALANCE_ACTIVE) &&
        (h5_app_abs(g_app.ball.error_cm) <=
         H5_APP_BALL_READY_ERROR_CM) &&
        (h5_app_abs(g_app.ball.velocity_cm_per_s) <=
         H5_APP_BALL_READY_SPEED_CM_S);
    ...
    if ((now_ms - g_app.ball_settle_start_ms) >=
        H5_APP_BALL_READY_SETTLE_MS) {
        g_app.ball_settled = true;
        g_app.state = H5_APP_READY;
    }
}

static void h5_build_telemetry_sample(
    const chassis_status_t *chassis,
    h5_telemetry_sample_t *sample)
{
...
static void h5_record_telemetry(
    const chassis_status_t *chassis, bool force)
{
    h5_telemetry_sample_t sample;
    ml_status_t status;

    if (!h5_telemetry_session_active()) {
        return;
    }
    h5_build_telemetry_sample(chassis, &sample);
    status = h5_telemetry_record(&sample, force);
    if (status == ML_STATUS_OK) {
...
static void h5_finish_telemetry(const chassis_status_t *chassis)
{
    h5_telemetry_sample_t sample;
    bool ball_passed = !g_app.ball_violation &&
        g_app.mission_output.score_point_passed;

    h5_telemetry_set_result(g_app.mission_output.score_elapsed_ms,
        g_app.maximum_score_error_cm, ball_passed);
    if (h5_telemetry_session_active()) {
        h5_build_telemetry_sample(chassis, &sample);
        h5_telemetry_session_finish(&sample);
    }
}

static void h5_start_recenter(h5_app_fault_t fault,
    const chassis_status_t *chassis)
{
    ...
    h5_finish_telemetry(chassis);
    g_app.state = H5_APP_RECENTERING;
    g_app.display_dirty = true;
}

static ml_status_t h5_start_run(chassis_status_t *status)
{
    ...
    *status = chassis_get_status();
    ...
    result = h5_mission_start(&g_app.mission, g_app.mode,
    ...
    g_app.mission_output.state = H5_MISSION_RUNNING;
    chassis_track_line_control_reset(&g_app.line_control);
    memset(&g_app.line_output, 0, sizeof(g_app.line_output));
    g_app.maximum_score_error_cm = h5_app_abs(g_app.ball.error_cm);
    g_app.interval_error_min_cm = g_app.ball.error_cm;
    g_app.interval_error_max_cm = g_app.ball.error_cm;
    ...
    g_app.velocity_started = false;
    g_app.h5_heading_only_active = false;
    result = h5_configure_line_control(g_app.mode);
    if (result != ML_STATUS_OK) {
        return result;
    }
    g_app.last_control_ms = status->timestamp_ms -
        g_h5_mission_default_config.control_period_ms;
    h5_telemetry_session_start(g_app.mode, status->timestamp_ms);
    h5_start_launch_bias(status->timestamp_ms);
    g_app.state = H5_APP_RUNNING;
    return ML_STATUS_OK;
}

static void h5_update_score_and_safety(
    const chassis_status_t *chassis)
{
    float absolute_error = h5_app_abs(g_app.ball.error_cm);

    if (!g_app.score_frozen) {
        if (absolute_error > g_app.maximum_score_error_cm) {
            g_app.maximum_score_error_cm = absolute_error;
        }
        if (absolute_error >= H5_APP_BALL_SCORE_IMMEDIATE_CM) {
            g_app.ball_violation = true;
        } else if (absolute_error > H5_APP_BALL_SCORE_ERROR_CM) {
    ...
    if (absolute_error > H5_APP_BALL_ABORT_ERROR_CM) {
        if (!g_app.ball_abort_active) {
            ...
            g_app.ball_abort_start_ms = chassis->timestamp_ms;
                   ...
                   H5_APP_BALL_ABORT_HOLD_MS) {
            h5_start_recenter(H5_APP_FAULT_BALL_ERROR, chassis);
        }
    } else {
        g_app.ball_abort_active = false;
    }
...
static void h5_run_control(chassis_status_t *status)
{
    ...
    g_app.last_line = line_sensor_read();
    if (g_app.last_line.io_fault) {
        h5_start_recenter(H5_APP_FAULT_LINE_GPIO, status);
        return;
    }
    center_distance_mm = 0.5f *
        (status->pose.left_distance_mm + status->pose.right_distance_mm);
    if ((center_distance_mm - g_app.mission.start_distance_mm >=
         H5_PASS_GATE_ARM_PROGRESS_MM) &&
        (g_app.last_line.black_bits == 0x0FU)) {
        h5_mission_mark_a_line(&g_app.mission);
    }
    command_status = h5_mission_update(&g_app.mission,
    ...
    if (command_status != ML_STATUS_OK) {
        h5_start_recenter(H5_APP_FAULT_CHASSIS, status);
        return;
    }
    if (g_app.mission_output.state ==
        H5_MISSION_FAULT_LAP_GATE) {
        h5_start_recenter(H5_APP_FAULT_LAP_GATE, status);
        return;
    }
    if (g_app.mission_output.score_point_passed) {
        g_app.score_frozen = true;
    ...
    if (g_app.mission_output.finished) {
        ...
        g_app.velocity_started = false;
        h5_clear_launch_bias();
        h5_finish_telemetry(status);
        g_app.state = H5_APP_FINISHED;
        g_app.display_dirty = true;
        return;
    }
    ...
    heading_abs_deg = h5_app_abs(g_app.mission_output.heading_progress_deg);
    (void) heading_abs_deg;
    g_app.h5_heading_only_active = false;
    request.heading_only = g_app.h5_heading_only_active;
...
ml_status_t h5_app_init(void)
{
    ...
    if (status == ML_STATUS_OK) {
        status = uart_init(UART0,
            H5_APP_UART_BAUD, H5_APP_UART_PRIORITY);
    }
    if (status == ML_STATUS_OK) {
        status = h5_telemetry_init();
    }
    if (status == ML_STATUS_OK) {
        status = chassis_init(&g_chassis_race_config);
    }
    if (status == ML_STATUS_OK) {
        status = h5_mission_init(&g_app.mission,
            &g_h5_mission_default_config);
    }
    if (status == ML_STATUS_OK) {
        failure_detail = "LINE CFG FAIL";
        status = h5_configure_line_control(g_app.mode);
    }
    if (status == ML_STATUS_OK) {
        failure_detail = "CHECK MODULES";
        status = icm42688_service_init(
            &g_app.imu, &g_h5_imu_config);
    }
    if (status == ML_STATUS_OK) {
        status = ball_balance_init();
    }
    if (status == ML_STATUS_OK) {
        g_app.target_cm = H5_TARGET_CM;
        status = ball_balance_set_target_cm(H5_TARGET_CM);
    }
    if (status == ML_STATUS_OK) {
    ...
    if (status != ML_STATUS_OK) {
        chassis_emergency_stop();
        g_app.fault = H5_APP_FAULT_INIT;
        g_app.state = H5_APP_FAULT;
        h5_show_init_failure(status, failure_detail);
        return status;
    }
    line_sensor_white_guard_reset(&g_app.white_guard);
    g_app.state = H5_APP_CALIBRATING;
    g_app.initialized = true;
    g_app.display_dirty = true;
    h5_require_key_release();
    (void) OLED_Clear();
    return ML_STATUS_OK;
}
...
void h5_app_poll(void)
{
    ...
    chassis_poll();
    status = chassis_get_status();
    h5_update_keys(status.timestamp_ms);
    ...
    if ((g_app.state == H5_APP_SETUP) ||
        ...
        /* Direction keys are deliberately consumed and never start motion. */
        (void) chassis_key_take_press(&g_app.up_key);
        (void) chassis_key_take_press(&g_app.down_key);
        (void) chassis_key_take_press(&g_app.left_key);
        (void) chassis_key_take_press(&g_app.right_key);
        h5_update_ball_settle(status.timestamp_ms);
        if ((g_app.state == H5_APP_READY) && center_press) {
            command_status = h5_start_run(&status);
            if (command_status != ML_STATUS_OK) {
                h5_start_recenter(H5_APP_FAULT_CHASSIS, &status);
            }
        }
    } else if ((g_app.state == H5_APP_RUNNING) && center_press) {
        h5_start_recenter(H5_APP_FAULT_EMERGENCY, &status);
    } else if ((g_app.state == H5_APP_FINISHED) && center_press) {
        h5_clear_launch_bias();
        (void) ball_balance_enable(false);
        g_app.state = H5_APP_RECENTERING;
        g_app.display_dirty = true;
    ...
    if (g_app.state == H5_APP_RUNNING) {
        h5_update_launch_bias(status.timestamp_ms);
        if ((status.result == CHASSIS_RESULT_FAULT) ||
            status.emergency_stop_latched) {
            h5_start_recenter(H5_APP_FAULT_CHASSIS, &status);
        } else if (g_app.ball.breakaway_fault) {
            h5_start_recenter(
                H5_APP_FAULT_BALL_CONTROL, &status);
        } else if (!g_app.ball.vision_ready || !g_app.ball.enabled) {
            h5_start_recenter(H5_APP_FAULT_VISION, &status);
        } else {
            h5_update_score_and_safety(&status);
            if (g_app.state == H5_APP_RUNNING) {
                h5_run_control(&status);
            }
            if (g_app.state == H5_APP_RUNNING) {
                h5_record_telemetry(&status, false);
            }
        }
    }

    if (g_app.state == H5_APP_RECENTERING) {
        if ((g_app.ball.servo_target_us == BALL_SERVO_CENTER_US) &&
            (g_app.ball.servo_current_us == BALL_SERVO_CENTER_US)) {
            g_app.state = H5_APP_EXPORT_READY;
            g_app.display_dirty = true;
        }
    }

    while (uart_try_read(UART0, &uart_byte) == ML_STATUS_OK) {
        (void) h5_telemetry_uart0_handle_byte(uart_byte,
            g_app.state == H5_APP_EXPORT_READY,
            status.timestamp_ms);
    }
...
ml_status_t h5_app_get_status(h5_app_status_t *status)
{
    ...
    status->telemetry_count = h5_telemetry_count();
    ...
    status->score_point_passed =
        g_app.mission_output.score_point_passed;
    status->export_allowed = g_app.state == H5_APP_EXPORT_READY;
    status->oled_frozen = g_app.state == H5_APP_RUNNING;
    return ML_STATUS_OK;
}
