...
#define H5_TELEMETRY_LINE_SIZE (320U)
#define H5_LINE_USABLE_MASK    (0x01U)
#define H5_LINE_RECOVERING_MASK (0x02U)
#define H5_BALL_VISION_MASK    (0x01U)
#define H5_BALL_ENABLED_MASK   (0x02U)
#define H5_BALL_VIOLATION_MASK (0x04U)
#define H5_BALL_BREAKAWAY_MASK (0x08U)
#define H5_SCORE_PASSED_MASK   (0x10U)
#define H5_HEADING_GATE_MASK   (0x20U)
...
typedef struct {
    ...
    uint8_t ball_flags;
    uint8_t mode_state;
} h5_telemetry_record_t;

typedef char h5_telemetry_record_must_be_44_bytes[
    (sizeof(h5_telemetry_record_t) == H5_TELEMETRY_RECORD_BYTES) ?
    1 : -1];

static h5_telemetry_record_t g_records[H5_TELEMETRY_CAPACITY];
...
static bool h5_telemetry_float_valid(float value)
{
    return (value == value) && (value <= FLT_MAX) &&
        (value >= -FLT_MAX);
}

static int32_t h5_telemetry_round(float value)
{
    return (int32_t) (value + ((value >= 0.0f) ? 0.5f : -0.5f));
}

static int16_t h5_telemetry_i16(float value, float scale)
{
    float scaled = value * scale;

    if (!h5_telemetry_float_valid(value) ||
    ...
    return (int16_t) h5_telemetry_round(scaled);
}

static int8_t h5_telemetry_i8(float value, float scale)
{
    float scaled = value * scale;
    ...
    return (int8_t) h5_telemetry_round(scaled);
}

static uint16_t h5_telemetry_u16(float value, float scale)
{
    float scaled = value * scale;

    if (!h5_telemetry_float_valid(value) || (scaled <= 0.0f)) {
        return 0U;
    }
...
static uint16_t h5_telemetry_u32_to_u16(uint32_t value)
{
    return value > UINT16_MAX ? UINT16_MAX : (uint16_t) value;
}

static void h5_telemetry_pack(h5_telemetry_record_t *record,
    const h5_telemetry_sample_t *sample)
{
    record->time_ms = sample->timestamp_ms - g_start_time_ms;
    record->progress_mm = h5_telemetry_u16(sample->progress_mm, 1.0f);
    record->fused_heading_cdeg =
        h5_telemetry_u16(sample->fused_heading_deg, 100.0f);
    record->target_center_dmm_s =
        h5_telemetry_i16(sample->target_center_mm_s, 10.0f);
    record->actual_center_dmm_s =
        h5_telemetry_i16(sample->actual_center_mm_s, 10.0f);
    ...
    record->line_flags =
        (sample->line_usable ? H5_LINE_USABLE_MASK : 0U) |
        (sample->line_recovering ? H5_LINE_RECOVERING_MASK : 0U);
    record->line_correction_mm_s =
        h5_telemetry_i8(sample->line_correction_mm_s, 1.0f);
    record->final_steering_bias_mm_s =
        h5_telemetry_i8(sample->final_steering_bias_mm_s, 1.0f);
    record->ball_target_milli_cm =
        h5_telemetry_i16(sample->ball_target_cm, 1000.0f);
    record->ball_position_milli_cm =
        h5_telemetry_i16(sample->ball_position_cm, 1000.0f);
    record->ball_error_min_deci_cm =
        h5_telemetry_i8(sample->ball_error_min_cm, 10.0f);
    record->ball_error_max_deci_cm =
        h5_telemetry_i8(sample->ball_error_max_cm, 10.0f);
    record->ball_velocity_centi_cm_s =
        h5_telemetry_i16(sample->ball_velocity_cm_s, 100.0f);
    record->ball_control_deci_us =
        h5_telemetry_i16(sample->ball_control_output_us, 10.0f);
    ...
    record->expected_heading_cdeg =
        h5_telemetry_u16(sample->expected_heading_deg, 100.0f);
    record->heading_error_cdeg =
        h5_telemetry_i16(sample->heading_error_deg, 100.0f);
    record->vision_age_ms =
        h5_telemetry_u32_to_u16(sample->vision_age_ms);
    record->ball_flags =
        (sample->vision_ready ? H5_BALL_VISION_MASK : 0U) |
        (sample->ball_enabled ? H5_BALL_ENABLED_MASK : 0U) |
        (sample->ball_violation ? H5_BALL_VIOLATION_MASK : 0U) |
        (sample->breakaway_fault ? H5_BALL_BREAKAWAY_MASK : 0U) |
        (sample->score_point_passed ? H5_SCORE_PASSED_MASK : 0U) |
        (sample->heading_gate_met ? H5_HEADING_GATE_MASK : 0U);
    record->mode_state = ((uint8_t) sample->mode & 0x0FU) |
        (uint8_t) (((uint8_t) sample->mission_state & 0x0FU) << 4U);
}

static uint16_t h5_append_char(
    char *buffer, uint16_t index, char value)
{
    if (index < H5_TELEMETRY_LINE_SIZE) {
        buffer[index++] = value;
    }
    return index;
}

static uint16_t h5_append_u32(
    char *buffer, uint16_t index, uint32_t value)
{
    ...
    while (count > 0U) {
        index = h5_append_char(buffer, index, digits[--count]);
    }
    return index;
}

static uint16_t h5_append_i32(
    char *buffer, uint16_t index, int32_t value)
{
    uint32_t magnitude;

    if (value < 0) {
        index = h5_append_char(buffer, index, '-');
        magnitude = (uint32_t) (-(value + 1)) + 1U;
    } else {
        magnitude = (uint32_t) value;
    }
    return h5_append_u32(buffer, index, magnitude);
}

static uint16_t h5_append_fixed(char *buffer, uint16_t index,
    int32_t value, uint32_t scale, uint8_t decimals)
{
    ...
    index = h5_append_u32(buffer, index, magnitude / scale);
    index = h5_append_char(buffer, index, '.');
    divisor = scale / 10U;
...
static ml_status_t h5_uart_write(const char *data, uint16_t length)
{
    uint16_t index;
...
static ml_status_t h5_export_csv(void)
{
    static const char header[] =
        ...
        "heading_gate_met,target_center_mm_s,actual_center_mm_s,"
        ...
        "servo_target_us,servo_current_us,raw_x_px,vision_age_ms,"
        "vision_ready,ball_enabled,ball_violation,"
        "breakaway_fault\r\n";
    char line[H5_TELEMETRY_LINE_SIZE];
    uint16_t record_index;
    ml_status_t status;

    status = h5_uart_write(header,
        (uint16_t) (sizeof(header) - 1U));
    for (record_index = 0U; (record_index < g_record_count) &&
         (status == ML_STATUS_OK); ++record_index) {
        const h5_telemetry_record_t *record =
            &g_records[record_index];
        uint16_t index = 0U;

#define H5_CSV_COMMA() \
        do { index = h5_append_char(line, index, ','); } while (0)
        index = h5_append_u32(line, index, record->time_ms);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            record->mode_state & 0x0FU);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            (record->mode_state >> 4U) & 0x0FU);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index, g_score_elapsed_ms);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            g_ball_score_passed ? 1U : 0U);
        H5_CSV_COMMA();
        index = h5_append_fixed(line, index,
            g_maximum_ball_error_milli_cm, 1000U, 3U);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            (record->ball_flags & H5_SCORE_PASSED_MASK) ? 1U : 0U);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index, record->progress_mm);
        H5_CSV_COMMA();
        index = h5_append_fixed(line, index,
        ...
        H5_CSV_COMMA();
        index = h5_append_fixed(line, index,
            record->heading_error_cdeg, 100U, 2U);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            (record->ball_flags & H5_HEADING_GATE_MASK) ? 1U : 0U);
        H5_CSV_COMMA();
        index = h5_append_fixed(line, index,
        ...
        H5_CSV_COMMA();
        index = h5_append_u32(line, index, record->pwm_left_count);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index, record->pwm_right_count);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index, record->line_bits);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            (record->line_flags & H5_LINE_USABLE_MASK) ? 1U : 0U);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            (record->line_flags & H5_LINE_RECOVERING_MASK) ? 1U : 0U);
        H5_CSV_COMMA();
        index = h5_append_i32(line, index,
            record->line_correction_mm_s);
        H5_CSV_COMMA();
        index = h5_append_i32(line, index,
            record->final_steering_bias_mm_s);
        ...
        H5_CSV_COMMA();
        index = h5_append_u32(line, index, record->servo_target_us);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index, record->servo_current_us);
        H5_CSV_COMMA();
        index = h5_append_i32(line, index, record->raw_x_px);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index, record->vision_age_ms);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            (record->ball_flags & H5_BALL_VISION_MASK) ? 1U : 0U);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            (record->ball_flags & H5_BALL_ENABLED_MASK) ? 1U : 0U);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            (record->ball_flags & H5_BALL_VIOLATION_MASK) ? 1U : 0U);
        H5_CSV_COMMA();
        index = h5_append_u32(line, index,
            (record->ball_flags & H5_BALL_BREAKAWAY_MASK) ? 1U : 0U);
        index = h5_append_char(line, index, '\r');
        index = h5_append_char(line, index, '\n');
#undef H5_CSV_COMMA
        status = h5_uart_write(line, index);
    }
    return status;
}
...
ml_status_t h5_telemetry_init(void)
{
    ...
    g_record_count = 0U;
    g_period_ms = H5_TELEMETRY_H5_PERIOD_MS;
    g_start_time_ms = 0U;
    ...
    h5_mode_t mode, uint32_t start_time_ms)
{
    ...
    (void) mode;
    g_period_ms = H5_TELEMETRY_LAP_PERIOD_MS;
    g_score_elapsed_ms = 0U;
    g_maximum_ball_error_milli_cm = 0;
    g_ball_score_passed = false;
    ...
    const h5_telemetry_sample_t *sample, bool force)
{
    ...
    h5_telemetry_pack(&packed, sample);
    if (g_record_count >= H5_TELEMETRY_CAPACITY) {
        g_buffer_full = true;
        if (!force) {
            return ML_STATUS_BUFFER_FULL;
        }
        g_records[H5_TELEMETRY_CAPACITY - 1U] = packed;
        return ML_STATUS_OK;
    }
    ...
    const h5_telemetry_sample_t *sample)
{
    if (g_session_active && (sample != 0)) {
        (void) h5_telemetry_record(sample, true);
    }
    g_session_active = false;
}
    ...
    float maximum_ball_error_cm, bool ball_score_passed)
{
    ...
    g_maximum_ball_error_milli_cm =
        h5_telemetry_i16(maximum_ball_error_cm, 1000.0f);
    ...
    uint8_t byte, bool export_allowed, uint32_t now_ms)
{
    ...
    {
        ml_status_t status = h5_export_csv();
