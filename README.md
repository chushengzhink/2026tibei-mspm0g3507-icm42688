...
纯LF04模式不启用实验性弯道方向记忆。B1/B8仍立即使用35%/120 mm/s增强，B0继续保持最后确认的质心误差；任意新有效位型在现有反向确认规则后立即接管。实测表明原弯道记忆可能在直线入口继续输出`+89～+120 mm/s`并造成持续转向，因此31%/110 mm/s保持量和1200 mm退出门只保留为未启用的实验配置。正式竞速使用同一LF04 PD作为方向优先输入，并由编码器里程和IMU融合航向提供不能反向覆盖红外的路线辅助。
