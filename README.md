...
正式竞速上电时不要按住上键，READY首行必须显示`RACE FUSION`。任务按中键边沿把当前位置和航向定义为A点零点并顺时针启动，路线为1.5 m直线、R500 mm半圆、1.5 m直线、R500 mm半圆，总长6141.6 mm。stage 0直线/弯道均为360 mm/s；通过实车验收后可用`CHASSIS_TRACK_SPEED_STAGE=1/2`把直线依次提高到380/400 mm/s，弯道保持360 mm/s。加减速度400 mm/s²，末段降至100 mm/s。

编码器里程生成AB/BC/CD/DA的期望航向和R500前馈，融合航向误差以`Kp=4.0`和`±0.35 rad/s`限幅修正，现有速度模式继续提供融合角速度反馈。终点减速区从剩余190 mm开始；只有编码器中心里程达到`6141.6-36 mm`且累计融合顺时针航向连续3个20 ms周期进入`360°±5°`，才允许制动。到`route+50 mm`仍未同时满足则锁停并显示`FAULT LAP CHECK`。
