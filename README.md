...
- 10 Hz RAM遥测可保存600条，每条44字节；记录累计航向、速度、实际PWM、LF04纠偏量和可用/恢复/异常状态，并可在停车后通过UART0导出21列CSV。
...
LF04从左到右为PA31/PA12/PB8/PA27，探头中心相对模块中心为`-40.25/-7.25/+7.25/+40.25 mm`。所有低电平探头按`error=-average(position)/40.25`计算归一化四路质心：左侧黑线得到正误差并使左轮减速、右轮加速，右侧完全对称；B6、B9、B15质心为0。所有`B1-B15`均可用，只有`B0`表示丢线。控制器每20 ms执行`Kp=1.2、Ki=0、Kd=0.20`的PD控制；首个有效样本预装D历史，避免启动冲击。普通位型采用当前中心速度的22%纠偏且总偏置不超过90 mm/s；B1/B8最外侧单探头仍立即采用35%纠偏和120 mm/s总偏置上限。左右误差直接反向时首周期把有效误差置0，连续第二周期才接受；居中和同侧变化不延迟，随后的B0可作为第二次确认并保持新方向。正式竞速中编码器/IMU轮间偏置只在与非零红外误差同向时叠加，反向辅助直接丢弃。B0不降速、不锁停，持续复用最后确认误差和B1/B8增强状态；尚无有效样本时误差为0且不增强。红外不参与终点双门，运行中中键仍立即急停。

纯LF04模式额外启用独立弯道记忆：经确认的B1/B8分别锁存左右曲率，弯道基础偏置与当前质心PD残差分开计算，B6/B9保持`min(速度×0.31,110)`，B15清除记忆。记忆态B0前600 ms保持`min(速度×0.35,120)`，随后600 ms线性过渡到保持值，之后无限保持；350 mm/s对应`120→108.5 mm/s`，不改变中心速度。控制器从首次B1/B8起按请求速度积分弯道行程：1200 mm以前，对侧内探头只减小当前弯道偏置，不能清除或反转记忆；达到1200 mm后还须连续5个20 ms周期确认对侧非外探头才退出，B0不能完成退出。正式竞速不启用这组记忆参数。
...
UART0固定为PA10/PA11、115200、8N1、无流控。竞速或纯LF04模式完全停稳后发送单个`D`导出纯21列CSV，确认文件保存后再发送`C`清空RAM；运行和制动期间两者均不执行，仅每秒最多回复一次`BUSY`。新增`line_correction_mm_s`位于`line_bits`之后，纯红外中表示实际红外轮间偏置。纯红外中键锁停会结束遥测会话并写入纠偏为0的最终快照。编码器和融合航向为不回绕的累计值，原始IMU Yaw保留回绕诊断；同时间戳立即采样覆盖上一条，导出的时间戳严格递增。SSCOM必须关闭“发→/收←”方向标记、时间标签、定时发送和自动换行，只发一次ASCII `D`或十六进制`44`并等待完整导出。
